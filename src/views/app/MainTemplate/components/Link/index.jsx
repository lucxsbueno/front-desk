...
const Link = props => {
  ...
  const isActive = (active || !!toggle)
    ? "main-template__link--active"
    : "";

  const isToggleActive = toggle
    ? "main-template__item--active"
    : "";

  return (
    <li className={`main-template__item ${isToggleActive}`.trim()}>
      <a href={to} onClick={() => setToggle(!toggle)} className={`main-template__link ${isActive}`.trim()}>
